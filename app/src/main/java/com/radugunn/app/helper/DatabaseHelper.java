package com.radugunn.app.helper;
...
import com.radugunn.app.model.SearchLive;
import com.radugunn.app.model.WishList;
import com.radugunn.app.utils.Debug;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TABLE_CART = "CREATE TABLE "
            ...
            + KEY_STOCK_QUANTITY + " INTEGER,"
            + KEY_MANAGE_STOCK + " TEXT,"
            + KEY_PRODUCT + " TEXT,"
            + KEY_VARIATION + " TEXT,"
            + KEY_VARIATION_ID + " INTEGER,"
            + KEY_BUY_NOW + " INTEGER,"
            + KEY_NOTE + " TEXT" + ")";
...
//        Cursor c = db.rawQuery(selectQuery, null);
    ...
    public long addToCart(Cart cart) {
        if (cart.getBuyNow() == 1) {
            ...
            values.put(KEY_NOTE, cart.getNote());

            // insert row
            long cart_id = db.insert(TABLE_CART, null, values);
            Log.e("Harsh", "addToCart: " + cart_id + "," + cart.getNote());
            if (db.isOpen())
                db.close();
            return cart_id;
        } else {
            if (getProductFromCart(cart)) {
                Log.e("TAG", "addToCart: if "+"called");
                ...
                // insert row
                long cart_id = db.insert(TABLE_CART, null, values);
                Log.e("TAG", "addToCart: insert 2: "+ cart_id + "," + cart.getNote() );
                if (db.isOpen())
                    db.close();
                return cart_id;
            }
    ...
    public long addVariationProductToCart(Cart cart) {
        ...
        if (getVariationProductFromCart(cart)) {
            ...
            // insert row
            long cart_id = db.insert(TABLE_CART, null, values);
            Log.e("TAG", "addToCart: insert 3: "+ cart_id +","+ cart.getNote() );
            if (db.isOpen())
                db.close();
            return cart_id;
        }
    ...
    public CategoryList getProductFromCartById(String product_id) {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                cart.setNote((c.getString(c.getColumnIndex(KEY_NOTE))));
    ...
    public List<Cart> getProductQuantity(int product_id) {
